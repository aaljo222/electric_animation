...
const createHelixCurve = () => {
  ...
  // 오른쪽(+)에서 왼쪽(-)으로 감겨 들어가는 나선
  // 300개의 점으로 부드러운 곡선 생성
  for (let i = 0; i <= 400; i++) {
    const t = i / 400;
...
const ElectronFlow = ({ curve }) => {
  ...
  useFrame(({ clock }) => {
    const t = clock.getElapsedTime() * 0.3; // 흐름 속도

    particlesRef.current.forEach((mesh, i) => {
      if (mesh) {
        // 0.0 ~ 1.0 사이의 균일한 간격 (offset)
        // (t + i / numParticles) % 1 : 입자들이 줄지어 이동하는 공식
  ...
  return (
    <group ref={fluxRef}>
      ...
      <Html position={[-3, 0, 0]} center>
        <div className="text-4xl font-black text-cyan-500 drop-shadow-md">
          N
        </div>
      </Html>
      <Html position={[3, 0, 0]} center>
        <div className="text-4xl font-black text-blue-700 drop-shadow-md">
          S
        </div>
      </Html>
    </group>
  ...
  return (
    <group position={[0, -2.5, 0]}>
      ...
      <Html position={[1.5, 1, 0]} center>
        <div className="text-yellow-500 font-bold text-lg">↑ I</div>
      </Html>
      <Html position={[-1.5, 1, 0]} center>
        <div className="text-yellow-500 font-bold text-lg">↓ I</div>
      </Html>
